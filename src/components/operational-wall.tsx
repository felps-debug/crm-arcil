      ...
      <section className="op-hero" style={{ margin: "24px 28px" }}>
        ...
        <div className="op-hero-metrics">
          <motion.div className="op-metric-box" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="op-metric-value">{formatNumber(summary?.activeLeads ?? 0)}</div>
          </motion.div>
          <motion.div className="op-metric-box" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="op-main-grid" style={{ margin: "0 28px 28px" }}>
        ...
        <section className="op-panel">
          <div className="op-panel-header">
            ...
            <span className="op-panel-count">{summary?.pendingFollowups ?? 0}</span>
          </div>
          <div className="op-queue-list">
            {[
        ...
        <section className="op-panel">
          <div className="op-panel-header">
            ...
            <span className="op-panel-count">{inventory?.totalProducts ?? 0}</span>
          </div>
          <div className="op-queue-list">
            {[
              { id: 1, name: "Em estoque", severity: "info", count: inventory?.inStock ?? 0 },
              { id: 2, name: "Baixo estoque", severity: "warning", count: inventory?.lowStock ?? 0 },
              { id: 3, name: "Fora de estoque", severity: "danger", count: inventory?.outOfStock ?? 0 },
            ].map((item) => (
              <motion.div key={item.id} className="op-queue-row" data-severity={item.severity} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}>
                <div className="op-queue-label">{item.name}</div>
                <div className="op-queue-count">{item.count}</div>
        ...
        <section className="op-panel">
          ...
          <div className="op-ledger" style={{ gap: "8px", padding: "12px 0" }}>
            <div className="op-ledger-cell" style={{ minWidth: "auto", flex: 1 }}>
              ...
              <span className="op-ledger-value">{formatMoney(summary?.totalPortfolio ?? 0)}</span>
            </div>
            <div className="op-ledger-cell" style={{ minWidth: "auto", flex: 1 }}>
              <span className="op-ledger-label">Recebido</span>
              <span className="op-ledger-value" style={{ color: "#70d8a1" }}>{formatMoney(summary?.totalReceived ?? 0)}</span>
            </div>
            <div className="op-ledger-cell" style={{ minWidth: "auto", flex: 1 }}>
              <span className="op-ledger-label">Em risco</span>
              <span className="op-ledger-value" style={{ color: "#ff7465" }}>{formatMoney(summary?.atRisk ?? 0)}</span>
            </div>
          </div>
        </section>
      </div>
